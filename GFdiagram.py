...
test1 = -(fi+12*fj**2+6*fj*(fi*fj+fi+fj**2)+7*fj+1)/8/(wi+wj)
test2 = (fi+6*fj*(fi*fj+fi-fj**2-fj)-fj)/8/(wi-wj)
pp(test2+test1)
pp(diag2D_2)
...
PT1 = -(fi+fj+8*fl*(fi*fl+fi+fj*fl+fj+fl+1)+1)/16/(wi+wj)
PT2 = (fi-fj+8*fl*(fi*fl+fi-fj*fl-fj))/16/(wi-wj)
diag2D = - sym.Rational(1,24)*(D2+D3+D8+D7)
diag2B = -sym.Rational(1,8)*(2*fk+1)*(2*fl+1)*(wi*(2*fj+1)-wj*(2*fi+1))/(wi**2-wj**2)
diag = (3*diag2D+diag2B/2).subs({fk:fl,wk:wl})
